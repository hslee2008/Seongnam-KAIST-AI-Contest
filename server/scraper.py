...
def scrape_pangyomuseum_events(max_pages=5):
    ...
    while page <= max_pages:
        ...
        try:
            ...
            for notice in notice_list:
                ...
                absolute_link = urljoin(base_url, f"/mobile/sub/{relative_link}")

                date_span = notice.select_one("p.info span")
                date_str = date_span.get_text(strip=True).replace("행사일 :", "").strip() if date_span else ""
                
                image_tag = notice.select_one("p.image img")
                image_src = urljoin(base_url, image_tag['src']) if image_tag else ""
...
def scrape_pangyonoin_events(max_pages=5):
...
def main():
    ...
    # --- 판교박물관 스크레이퍼 ---
    pangyomuseum_events = scrape_pangyomuseum_events()
    all_events.extend(pangyomuseum_events)
    # --- 판교종합사회복지관 스크레이퍼 ---
    pangyowelfare_events = scrape_pangyowelfare_events()
    all_events.extend(pangyowelfare_events)
    # --- 판교노인종합복지관 스크레이퍼 ---
    pangyonoin_events = scrape_pangyonoin_events()
